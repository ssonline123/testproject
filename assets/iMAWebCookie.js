...
(function() {
    ...
    w.ewt = w.ewt || {
        ...
        smartContent: function(elementId, contentUrl) {
            ...
            smartContentJsonp.src = endpointSmartContent() + "?contentUrl=" + contentUrl
                + "&visitorKey=" + getVisitorKeyForPersonalization()
                + "&orgGuid="+ pagekey()
                + "&elementId=" + elementId
                + "&callback=ewt.smartContentSuccess"
    ...
    function getFormActionUrl(formElement) {
        ...
        if (formElement.hasAttribute('action')) {
            formActionUrl = formElement.getAttribute('action');
            if (typeof(formActionUrl) != 'string' &&
                typeof(formElement.attributes.action.value) == 'string') {
                formActionUrl = formElement.attributes.action.value; //ie
            }
        }
